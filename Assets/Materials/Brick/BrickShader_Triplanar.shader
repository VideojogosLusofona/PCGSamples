Shader "PCGClass/BrickTexture_Triplanar"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Fade curve (Perlin's improved fade). (same as your GLSL)
           ...
           float2 gradAt(int2 p)
            ...
            {
                // --- Triplanar projection (object space) ---
                float3 p = IN.positionOS * _PlanarScale;
                float3 n = normalize(IN.normalOS);

                // Weights from the actual normal (sharpen with exponent).
                // Larger exponent -> closer to hard plane switching.
                float3 w = abs(n);
                w = max(w, 1e-4);

                // Harshness / sharpness
                if (_TriplanarBlendFactor > 20)
                {
                    if (w.x > w.y)
                    {
                        if (w.x > w.z) { w.x = 1; w.y = 0; w.z = 0; }
                        else { w.x = 0; w.y = 0; w.z = 1; }
                    }
                    else
                    {
                        if (w.y > w.z) { w.x = 0; w.y = 1; w.z = 0; }
                        else { w.x = 0; w.y = 0; w.z = 1; }
                    }
                }
                else
                {
                    w = pow(w, _TriplanarBlendFactor);
                ...
                // X-facing surfaces -> project onto the YZ plane
                float2 uvX = p.zy;
                // Y-facing surfaces -> project onto the XZ plane
                float2 uvY = p.xz;
                // Z-facing surfaces -> project onto the XY plane
                float2 uvZ = p.xy;

                // Evaluate the same procedural brick texture in each projection
                float3 colX = brickTexture(uvX);
                float3 colY = brickTexture(uvY);
                float3 colZ = brickTexture(uvZ);

                // Blend
                float3 col = colX * w.x + colY * w.y + colZ * w.z;
